...
cleanup() {
  ...
  if docker ps -a --filter "name=og-assignment-full" --format '{{.Names}}' | grep -q '^og-assignment-full$'; then
    echo "Stopping and removing application container..."
    docker stop og-assignment-full
    docker rm og-assignment-full
  fi
